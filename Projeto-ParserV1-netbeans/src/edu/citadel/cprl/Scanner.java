...
public class Scanner {
    ...
    public void advance() throws IOException {
        try {
            ...
            symbol = source.getChar() == Source.EOF
                ? Symbol.EOF
                : Symbol.unknown;
        }
    }
    ...
    private String scanIdentifier() throws IOException {
        ...
        do {
            ...
            Character.isAlphabetic((char) source.getChar()) ||
    ...
    private String scanEscapedChar() throws ScannerException, IOException {
        ...
        switch (c) {
            ...
            default:
                ...
                ScannerException ex = new ScannerException(
                    backslashPosition,
                    errMessage
                );
    ...
    private void checkGraphicChar(int n) throws ScannerException {
        if (n == Source.EOF) {
            ...
            if (c == '\r' || c == '\n') {
                throw error(
                    "Char and String literals can not extend past end of line."
                );
    ...
    public void advanceTo(Symbol[] symbols) throws IOException {
        while (true) {
            if (
                search(symbols, symbol) >= 0 || source.getChar() == Source.EOF
            ) {
                return;
            } else {
                advance();
