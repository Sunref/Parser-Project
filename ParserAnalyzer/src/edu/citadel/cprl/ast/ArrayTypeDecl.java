...
public class ArrayTypeDecl extends InitialDecl {
    ...
    public ArrayTypeDecl(Token typeId, Type elemType, ConstValue numElements) {
        super(
            ...
            new ArrayType(
                typeId.getText(),
                numElements.getLiteralIntValue(),
                elemType
            )
        );
    ...
    public void checkConstraints() {
        ...
        // Implementação:
        try {
            if (!numElements.getType().equals(Type.Integer)) {
                String errorMsg = "Array size must be an integer";
                throw error(numElements.getPosition(), errorMsg);
            } else if (numElements.getLiteralIntValue() <= 0) {
